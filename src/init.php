...
function gutenberg_extra_load_custom_wp_admin_script() {
	...
	// Inline feedback script
	wp_register_script('maxi-blocks-feedback-script', false);
	wp_add_inline_script(
		'maxi-blocks-feedback-script',
		"Userback = window.Userback || {};
		Userback.access_token = '9457|18147|FC9chXgenIsR4uvctylhyvdDUG8lONFvGLTuPRwULwnG7LREge';
		(function(id) {
...
/**
 * MaxiBlocks JS backend global object
 *
 * Generates a JS object that is stored as a global variable into the DOM with
 * information such as:
 * - WP version
 * - Gutenberg version
 */
if (!function_exists('maxi_global_object_generator')) {
	function maxi_global_object_generator() {
		global $wp_version;
		...
		if (defined('GUTENBERG_VERSION')) {
			...
			if (version_compare($wp_version, '5.5') >= 0 && version_compare($wp_version, '5.5.3') <= 0) {
				$version = '8.5';
			} elseif (version_compare($wp_version, '5.6') >= 0 || floatval($wp_version) >= 5.6) {
				$version = '9.2';
			}
		}
		...
		$encoded_global = wp_json_encode($global);

		return "<script>var maxiBlocksSettings = $encoded_global;	</script>";
	}

	add_action('admin_print_scripts', 'maxi_global_object_generator');
}
