...
function maxi_check_database_version()
{
    ...
    if (version_compare($wpdb->db_version(), $requiredVersion, '<')) {
        ...
        add_action(
            ...
            'maxi_show_database_version_notice',
            10,
            3,
        );
        add_action(
            'admin_enqueue_scripts',
            'maxi_blocks_enqueue_notice_scripts',
        );
    }
}
...
{
    wp_enqueue_script(
        'maxi-blocks-admin-notice',
        plugins_url('/core/admin/notices.js', __FILE__),
        [],
        MAXI_PLUGIN_VERSION,
        true,
    );
...
function maxi_show_database_version_notice()
{
    ...
    $message =
        sprintf(
            ...
            esc_html__(
                'Highly recommend to update to %1$s %2$s+ for enhanced security, better performance, and full feature compatibility.',
                'maxi-blocks',
            ),
        ...
        ' <a href="' .
        esc_url('https://maxiblocks.com/go/database-version-requirements') .
        '" target="_blank">' .
        esc_html__('Learn more', 'maxi-blocks') .
        '</a>';
...
if (class_exists('MaxiBlocks_API')) {
    add_action('plugins_loaded', ['MaxiBlocks_API', 'register']);
}
...
if (class_exists('MaxiBlocks_PageTemplate')) {
    add_action('plugins_loaded', ['MaxiBlocks_PageTemplate', 'register']);
}
...
if (get_template() === 'maxiblocks-go') {
    ...
    if (version_compare($theme_version, '1.2.1', '>=')) {
        require_once MAXI_PLUGIN_DIR_PATH .
            'core/maxiblocks-go/maxiblocks-go.php';
    }
}
...
if (file_exists($env_file)) {
    ...
    if ($env_config !== false) {
        $is_test_environment =
            isset($env_config['GITHUB_ENV_TEST']) &&
            in_array(
                $env_config['GITHUB_ENV_TEST'],
                [1, '1', true, 'true'],
                true,
            );
    }
