	...
	class MaxiBlocks_Dashboard {
		...
		public function maxi_blocks_welcome() {
			...
			$content .=
				'<p>' .
				__(
					'Get new patterns, tips, and updates—in spam.',
					'maxi-blocks',
				) .
				'</p>';
			$content .=
		...
		public function maxi_blocks_settings() {
			...
			$description .=
				'<p>' .
				__(
					'<strong>You are currently using:</strong> ' .
						$font_provider_label,
				) .
				'</p>';
			$description .=
				'<p>' .
				__(
					'<strong>Bunny Fonts:</strong> Privacy-friendly, GDPR compliant. Global CDN for fast loading. Wide selection of fonts available.',
					'maxi-blocks',
				) .
				'</p>';
			$description .=
				'<p>' .
				__(
					'<strong>Google Fonts:</strong> Extensive font selection. Potential privacy concerns when using Google\'s CDN.',
					'maxi-blocks',
				) .
				'</p>';
			...
			if ($use_bunny_fonts) {
				...
				$description .=
					'<p>' .
					__(
						'<strong> Serve Bunny Fonts from CDN: </strong> Fastest option. Uses external CDN. No local storage required.',
						'maxi-blocks',
					) .
					'</p>';
				$description .=
					'<p>' .
					__(
						'<strong> Serve Bunny Fonts locally: </strong> Privacy-focused. May impact server performance. Requires local storage.',
						'maxi-blocks',
					) .
					'</p>';
				$content .= $this->generate_setting(
				...
				$description .=
					'<p>' .
					__(
						'<strong> Serve from Google CDN: </strong> Fastest option. Uses Google\'s CDN. Potential privacy (GDPR) implications.',
						'maxi-blocks',
					) .
					'</p>';
				$description .=
					'<p>' .
					__(
						'<strong> Serve Google Fonts locally: </strong> Blocks Google tracking. May impact server performance. Requires local storage.',
						'maxi-blocks',
					) .
					'</p>';
				$content .= $this->generate_setting(
