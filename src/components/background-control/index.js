...
const { SelectControl, ButtonGroup, Button } = wp.components;
...
import { isEmpty, isNil, pullAt } from "lodash";
...
export const backgroundControlAttributes = {
  backgroundOptions: {
    type: "string",
    default:
      '{"label":"Background","backgroundOptions":[{"imageOptions":{"mediaID":"","mediaURL":""},"sizeOptions":{"size":"auto","widthUnit":"%","width":100,"heightUnit":"%","height":100},"repeat":"repeat","positionOptions":{"position":"center center","widthUnit":"%","width":0,"heightUnit":"%","height":0},"origin":"padding-box","clip":"border-box","attachment":"scroll"}],"colorOptions":{"color":"","gradient":"","gradientAboveBackground":false},"blendMode":"normal"}',
  },
};
...
class BackgroundOptions extends Component {
  ...
  render() {
    const { backgroundOptions, onChange, onModalChange } = this.props;
        ...
        <div className="gx-backgroundoptions-tabs">
          <ButtonGroup>
            {backgroundOptions.map((option, i) => {
              return (
                <Button isSecondary onClick={() => onChangeBackground(i)}>
                  {i + 1}
                </Button>
              );
            })}
            <Button isSecondary onClick={onAddBackground}>
              +
            </Button>
          </ButtonGroup>
        </div>
        <Fragment>
          ...
          {backgroundOptions[selector].imageOptions.mediaID && (
            <Fragment>
              ...
              />
              {backgroundOptions[selector].positionOptions.position ===
                "custom" && (
                <Fragment>
                  <MiniSizeControl
                  ...
                  <MiniSizeControl
                    unit={
                      backgroundOptions[selector].positionOptions.heightUnit
                    }
                    onChangeUnit={(val) => {
                      backgroundOptions[
                        selector
...
class BackgroundControl extends Component {
  ...
  render() {
    ...
    let value =
      typeof backgroundOptions === "object"
        ? backgroundOptions
        : JSON.parse(backgroundOptions);

    console.log(value);
    const classNamePopover = `gx-background-popover gx-popover ${
      modalIsOpen ? " gx-background-popover-hide" : ""
    }`;
    ...
    const getBackgroundObject = () => {
      ...
      value.backgroundOptions.map((option) => {
        ...
        if (option.sizeOptions.size != "custom") {
          ...
          else response.general["background-size"] = option.sizeOptions.size;
        } else {
          if (!isNil(response.general["background-size"]))
            response.general["background-size"] = `${
              response.general["background-size"]
            }, ${option.sizeOptions.width + option.sizeOptions.widthUnit} ${
              option.sizeOptions.height + option.sizeOptions.heightUnit
            }`;
          else
            response.general["background-size"] = `${
              option.sizeOptions.width + option.sizeOptions.widthUnit
        ...
        if (option.positionOptions.position != "custom") {
          ...
          if (!isNil(response.general["background-position"]))
            response.general["background-position"] = `
              ...
              option.positionOptions.height + option.positionOptions.heightUnit
            }`;
          else
            response.general["background-position"] = `${
