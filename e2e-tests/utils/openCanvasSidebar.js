...
const openCanvasSidebar = async (page, item) => {
	await openDocumentSettingsSidebar();
	await ensureSidebarOpened();

	const sidebar = await page.$('.maxi-sidebar');

	const canvasOptions = await sidebar.$('.maxi-tabs-control');
	await canvasOptions.$$eval('button', canvasButtons =>
		canvasButtons[1].click()
	);
