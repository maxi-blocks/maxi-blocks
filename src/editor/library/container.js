 ...
 */
 ...
 const LibraryContainer = props => {
	 ...
	 useEffect(() => {
		 if (ref.current) {
			 ...
			 const breakEls = Array.from(
				 listWrapper.getElementsByClassName('break')
			 );
 
			 if (breakEls.length !== 4) {
				 breakEls.forEach(el => el.remove());
 
				 for (let i = 0; i < 4; i += 1) {
					 const breakEl = document.createElement('div');
					 breakEl.classList.add('ais-InfiniteHits-item');
					 breakEl.classList.add('break');
					 listWrapper.appendChild(breakEl);
				 }
			 }
		 }
	 });
	 ...
	 return (
		 <div className='maxi-cloud-container' ref={ref}>
			 {type === 'svg' && (
