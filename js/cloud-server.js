...
jQuery(document).ready(function ($) {
	...
	function onIframeLoad() {
		...
		let full_stop = 0;
		let id = $('.maxi-block-library__modal').attr('class').match(/maxi-block-id-+?.*/);

		if (id) {
			id = (id+'').replace('maxi-block-id-', '');
		}
				...
				if (e.origin === 'https://ge-library.dev700.com') { //'https://ge-library.dev700.com
					...
					if (jQuery.type(e.data) === 'object') { // check if the response is text
						...
						if (~e.data.json.indexOf('wp_block') && full_stop !== 1) { // if the response is a gutenberg json file
							...
							let clientId;
							if (id) clientId = id;
							else clientId = wp.data.select('core/block-editor').getSelectedBlock().clientId;
