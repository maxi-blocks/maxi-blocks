...
function get_shape_divider_styles($obj, $location) {
	...
	$raw_positions = get_margin_padding_styles([
		'obj' => [
			...get_group_attributes($obj, 'padding')
		]
	]);
...
function get_shape_divider_svg_styles($obj, $location, $blockStyle) {
	$response = [
	...
	foreach ($breakpoints as $breakpoint) {
		...
		} else if ($palette_status && $palette_color) {
