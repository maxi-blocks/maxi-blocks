    ...
    class MaxiBlocks_Dashboard
    {
        ...
        */
        ...
        public function maxi_register_menu()
        {
            ...
            add_submenu_page(self::$maxi_slug_dashboard, __(self::$maxi_plugin_name, self::$maxi_text_domain), __('Welcome', self::$maxi_text_domain), 'manage_options', self::$maxi_slug_dashboard, '', null);
        ...
        {
            $settings_tabs = array(
                self::$maxi_prefix.'welcome' => __('Welcome', self::$maxi_text_domain),
            ...
            if (isset($_GET['tab'])) { // phpcs:ignore
                ...
                $current_tab = $tab =  self::$maxi_prefix.'welcome';
            }
            ...
            if (isset($tab)) {
                if ($tab === self::$maxi_prefix.'welcome') {
        ...
        public function maxi_blocks_welcome()
        {
            ...
            $content .= '<h1>'.__('Welcome, ', self::$maxi_text_domain).esc_html($user_name).'</h1>';
            $content .= '<h2>'.__('Prototype web pages and tell your story with Maxi Blocks', self::$maxi_text_domain).'</h2>';
            
            $content .= '<p>'.__('Thank you for installing Maxi Blocks.', self::$maxi_text_domain).'</p>';
            $content .= '<p>'.__('Join us as we create the most fun and flexible page builder for WordPress.', self::$maxi_text_domain).'</p>';

            $content .= '<h3>'.__('Template library and Style Cards', self::$maxi_text_domain).'</h3>';
            $content .= '<p>'.__('Even the best designers use page templates to save time. The trick is to change styles without wasting hours choosing colours and fonts. You need a shortcut. You need a Style Card.', self::$maxi_text_domain).'</p>';
            $content .= '<p>'.__('Style Cards change 30 design elements in sync. It works like this:', self::$maxi_text_domain).'</p>';

            $content .= '<ol>';
            $content .= '<li>'.__('Choose your favourite Style Card from 100 shown in the library.', self::$maxi_text_domain).'</li>';
            $content .= '<li>'.__('Browse the template library to find a good page or pattern.', self::$maxi_text_domain).'</li>';
            $content .= '<li>'.__('Insert and watch how templates instantly match your chosen style.', self::$maxi_text_domain).'</li>';
            $content .= '</ol>';

            $content .= '<h3>'.__('Maxi Blocks is free and open source', self::$maxi_text_domain).'</h3>';
            $content .= '<p>'.__('Whatever you create with Maxi Blocks is yours to keep. You are welcome to use the free templates on as many sites as you want. Don’t forget to share your pages with the hashtag', self::$maxi_text_domain);
            $content .= ' <a href="https://twitter.com/#maxiblocks/" target="_blank">#maxiblocks</a> - '.__('We’re dying to see what you create.', self::$maxi_text_domain).'</p>';
            $content .= '<p>'.__('Our next goal is to launch the Maxi Blocks Pro template library subscription. Hundreds of patterns and pages have already been completed. It’s going to be epic. This income will help us grow the team and build out the awesome roadmap.', self::$maxi_text_domain);
        ...
        public function maxi_blocks_settings()
        {
            ...
            $description .= '<p>'.__('This is a requirement enforced by Google.', self::$maxi_text_domain).'</p>';
            ...
            $content .= '<a href="" target="_blank"> '.__('online documentation', self::$maxi_text_domain).'</a>';
            ...
            $content .= '<p><a href="" target="_blank"> '.__('Go to site health info', self::$maxi_text_domain).'</a></p>';
