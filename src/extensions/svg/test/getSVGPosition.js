...
import setSVGPosition from '@extensions/svg/setSVGPosition';
...
describe('getSVGAspectRatio', () => {
	...
	it('Should return the position of the image in SVG', () => {
		const aspectRatio = getSVGPosition(icon);

		expect(aspectRatio).toBe('xMidYMid');
	});

	it('Should return the position of the image in SVG after changing it', () => {
		const newIcon = setSVGPosition(icon, 'xMaxYMin');

		const aspectRatio = getSVGPosition(newIcon);

		expect(aspectRatio).toBe('xMaxYMin');
	});
});
