...
jQuery(document).ready(function ($) {
    ...
    wp.data.subscribe(function () {
        ...
        if (!isSavingPost && !isAutosavingPost) {
            let rb_to_send = '';
            let maxi_reusable_blocks = wp.data
                .select('core')
    ...
    $('.components-button.maxi-block-library__modal-button').on(
        'click',
        function () {
    ...
    function onIframeLoad() {
        ...
        if (typeof frame !== 'undefined' && frame !== null) {
                ...
                function (e) {
                    ...
                    if (e.origin === 'https://ge-library.dev700.com') {
                        ...
                        if (jQuery.type(e.data) === 'object') {
                            ...
                            ).removeClass('maxi-block__item--hidden');
                            if (~e.data.json.indexOf('wp_block')) {
                                // if the response is a gutenberg json file
                                ...
                                if (response) {
                                    ...
                                    let current_post_id = wp.data
                                        .select('core/editor')
                                        .getCurrentPostId();

                                    let results_uniq_json = JSON.stringify(
                                        results_uniq
                                    );
                                    ...
                                    jQuery.ajax({
                                        ...
                                        success: function (data) {
                                            ...
                                            let final_json_content =
                                                response.content;

                                            data_array.forEach(function (
                                                item,
                                                index
                                            ) {
                                                if (item !== '') {
                                                    ...
                                                    final_json_content = final_json_content.replace(
                                                        reg,
                                                        data_links_pair[1]
                                                    );
                                                    //console.log('REG' + reg);
                                                ...
                                                jQuery.ajax({
                                                    ...
                                                    success: function (data) {
                                                        ...
                                                        jQuery(
                                                            ...
                                                            'Saved to Re-usable Blocks Library'
                                                        );
                                                        setTimeout(function () {
                                                            jQuery(
                                                                '.maxi-block-library__modal__loading_message'
                                                            ).addClass(
                                                                'maxi-block__item--hidden'
                                                            );
                                                        }, 3000);
                                                    },
                                                    error: function (data) {
                                                        console.log(
                                                            'Re-usable import error'
                                                        );
                                                        console.log(data);
                                                    },
                                                }); //jQuery.ajax({
                                ...
                                final_fa_content =
                                    '<i class="maxi-fa-icon ' +
                                    e.data.fatype +
                                    ' ' +
                                    e.data.json +
                                    '"></i>';
                                final_fa_block =
                                    '<!-- wp:maxi-blocks/icon-maxi -->' +
                                    final_fa_content +
                                    '<!-- /wp:maxi-blocks/icon-maxi -->';
                                if (full_stop !== 1) {
                                    wp.data
            ...
            $('.maxi-cloud-modal .components-button.components-icon-button').on(
                'click',
                function () {
                    console.log('close');
