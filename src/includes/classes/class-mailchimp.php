 ...
 *
 * @package GXBlocks\Newsletter\Mailchimp
 */
...
final class Mailchimp implements Provider_Interface {
	...
	private function cached_lists() {
		...
		$lists = get_transient( 'gx_blocks_mailchimp_lists' );
		...
		if ( empty( $lists ) ) {
			...
			set_transient( 'gx_blocks_mailchimp_lists', $lists, MINUTE_IN_SECONDS * 15 );
		}
