(function ($) {
	...
	const { media } = wp;
	const SelectFrame = media.view.MediaFrame.Select;
	const PostFrame = media.view.MediaFrame.Post;
	const ManageFrame = media.view.MediaFrame.Manage;
	...
	const augmentFramePrototype = FrameConstructor => {
		if (!FrameConstructor) {
			return;
		}

		const originalCreateStates =
			FrameConstructor.prototype.createStates || function () {};
		FrameConstructor.prototype.createStates = function () {
		...
		const originalBrowseRouter =
			FrameConstructor.prototype.browseRouter || function () {};
		FrameConstructor.prototype.browseRouter = function (routerView) {
			originalBrowseRouter.apply(this, arguments);

			if (!routerView.get('maxi-unsplash')) {
				const frame = this;

				routerView.set('maxi-unsplash', {
		...
		const originalBindHandlers =
			FrameConstructor.prototype.bindHandlers || function () {};
		FrameConstructor.prototype.bindHandlers = function () {
			originalBindHandlers.apply(this, arguments);

			this.on(
				'content:create:maxi-unsplash',
				this.renderMaxiUnsplash,
				this
			);
			this.on(
				'content:render:maxi-unsplash',
				this.renderMaxiUnsplash,
				this
			);
			this.on(
				'toolbar:create:maxi-unsplash',
				this.createMaxiUnsplashToolbar,
				this
			);
		};
	};

	const assignRenderers = FrameConstructor => {
		if (!FrameConstructor) {
			return;
		}

		FrameConstructor.prototype.renderMaxiUnsplash = function (
			contentRegion
		) {
		...
		FrameConstructor.prototype.createMaxiUnsplashToolbar = function (
			toolbarRegion
		) {
	...
	augmentFramePrototype(SelectFrame);
	augmentFramePrototype(PostFrame);
	augmentFramePrototype(ManageFrame);

	assignRenderers(SelectFrame);
	assignRenderers(PostFrame);
	assignRenderers(ManageFrame);
	...
	const MaxiUnsplashBrowser = wp.Backbone.View.extend({
		...
		fetchResults(append) {
			...
			fetch(`${this.settings.ajaxUrl}?${queryArgs}`, {
