...
class MaxiBlocks_Styles
{
     ...
     * Custom Meta
     */
    public function customMeta($meta_js)
    {
        global $post;
        if (!$post || !isset($post->ID) || empty($meta_js)) {
            return;
        }
        ...
        if (!isset($result[$meta_js])) {
            return;
        }

        $result_decoded = $result[$meta_js];
    ...
    public function update_color_palette_backups($style)
    {
        ...
        foreach ($color_vars as $color_key => $color_value) {
            $star_pos = strpos($style_card, $color_key);
            $end_pos = strpos($style_card, ';--', $star_pos);
            $color_sc_value = substr(
                $style_card,
                $star_pos + strlen($color_key) + 1,
                $end_pos - $star_pos - strlen($color_key) - 1
            );
