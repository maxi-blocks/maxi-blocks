 ...
 */
const getLastBreakpointValue = (target, breakpoint, attributes = null) => {
	const { getBlockAttributes, getSelectedBlockClientId } = select(
		'core/block-editor'
	);

	const attr = attributes || getBlockAttributes(getSelectedBlockClientId());
	let currentAttr = attr[`${target}-${breakpoint}`];
	...
	do {
		...
		currentAttr = attr[`${target}-${breakpoints[breakpointPosition]}`];
	} while (
