...
function addGXLayoutButton() {
	...
	html += `<a type="button" id="maxi-button__layout" class="button components-button components-icon-button" aria-label="${ __( 'MaxiBlocks Library', 'maxi-blocks' ) }">\
	</i><img src="/wp-content/plugins/maxi-blocks/img/maxi-icon-white.svg" /> ${ __( 'MaxiBlocks Library', 'maxi-blocks') }</a>`;
	html += `<a type="button"  href="/wp-admin/customize.php" target="_blank" id="maxi-button__go-to-customizer" class="button components-button components-icon-button" aria-label="${ __( 'Global Styles', 'maxi-blocks' ) }">\
	</i><img src="/wp-content/plugins/maxi-blocks/img/maxi-icon-white.svg" /> ${ __( 'Global Styles', 'maxi-blocks') }</a>`;
	html += '</div>';
	buttonDiv.innerHTML = html;
	toolbar.appendChild( buttonDiv );
	document.getElementById( 'maxi-button__layout' ).addEventListener( 'click', abInsertLayout );
}
