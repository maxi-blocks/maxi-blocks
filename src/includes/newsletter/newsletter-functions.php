...
namespace GXBlocks\Newsletter;

use GXBlocks\Exception\Mailchimp_API_Error_Exception;

add_action( 'init', __NAMESPACE__ . '\form_submission_listener' );
add_action( 'wp_ajax_gx_blocks_newsletter_submission', __NAMESPACE__ . '\form_submission_listener' );
add_action( 'wp_ajax_nopriv_gx_blocks_newsletter_submission', __NAMESPACE__ . '\form_submission_listener' );
/**
...
function form_submission_listener() {
	...
	$default_attributes = gx_blocks_newsletter_block_attributes();
...
function process_submission( $email, $provider, array $args ) {
	...
	switch ( $provider ) {
		...
		case 'mailchimp':
			$api_key = get_option( 'gx_blocks_mailchimp_api_key' );
...
function mailing_list_providers() {
	...
	$mailchimp_api_key = get_option( 'gx_blocks_mailchimp_api_key' );
	...
	return [
		'mailchimp' => [
			...
			'api_key_defined' => ! empty( get_option( 'gx_blocks_mailchimp_api_key' ) ),
		],
	];
}
